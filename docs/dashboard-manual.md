...
Every chart below is a different cut of those four answers. The tables lower down are the model itself: one row per lead, and one row per individual call or text.
...
One vocabulary that Sales, Ops, Product and Engineering can all reconcile to. Read the cards left to right: north star, then its two drivers, then the business outcome, then the guardrail. Each card carries its target as a pill underneath, so you can see at a glance that the pipeline is currently failing four of its own standards.
...
A donut of the five outcomes, mutually exclusive, summing to 100%. Each slice is labelled with its count.
...
## Band 5: investigate a single lead

This is the basic investigative loop. A customer complains, a partner escalates, or a row in the exception queue looks odd. You have a lead ID, and one search shows you everything.

**Type a lead ID into the Lead ID filter** at the top of the dashboard. You can type the number alone (`20169`) or the full ID (`L-20169`). Every table on the page narrows to that lead: its ledger row, its position in the exception queue, and the attempt history below.

### Attempt history: every call and text for a lead

One row per outreach attempt, in order. Where the ledger's `what_happened` column summarises the sequence, this table breaks it apart so you can see each individual event and judge it.

| Column | Shows |
|---|---|
| `attempt_number` | Where this attempt sits in the sequence |
| `minutes_since_received` | How long after the lead arrived this happened |
| `channel`, `status` | The raw telecom record: call or text, and what the platform returned |
| `what_it_meant` | Contact made · Touch, no contact · **Not a touch at all (carrier failure)** |
| `should_have_triggered` | The action this event ought to have caused |
| `agent_id` | Who owned the attempt |
| `lead_local_hour` | The hour in the customer's own timezone, not ours |
| `outside_quiet_hours`, `sms_without_consent`, `after_connect` | Per-attempt risk flags |

**Why the last two content columns matter.** `what_it_meant` and `should_have_triggered` sit side by side with the raw status, so the failure and its fix appear on the same row. Search **L-20169** and you get three texts to a landline, seven minutes apart, each one reading "Not a touch at all (carrier failure)" next to "Switch to a call now, never retry SMS". Three logged events, three system successes, zero contact, and the correct next action never fired.

Two more worth searching:

- **L-20128** — a lead that stopped after a call and a text. The attempt history shows both touches landing normally and then nothing, which is what an abandoned cadence looks like at the event level.
- Any lead flagged **Duplicate / over-contact** in the queue — the history shows the attempt that happened *after* a completed call, with `after_connect` set.

### A note on the metric cards

Searching a single lead also narrows the six metric cards, so the north star will read 0% or 100% for that one lead. That is intended: it shows the metrics are computed per lead from the ledger rather than hardcoded. Clear the filter to return to the full population.

## Three things worth clicking

**1. Click the "Abandoned" slice.** Cross-filtering is on, so the ledger below filters to those 45 leads. Read the `what_happened` column: most show one call and nothing after, or a call and a text with no second call. The cadence stops mid-sequence, across many agents and every partner.
...
**3. Search a lead ID.** Take any row from the exception queue, drop its ID into the Lead ID filter, and read the attempt history. This is the loop an ops lead would run a dozen times a day.

**4. Sort the ledger by `sla_status`, then filter Partner to LeadBridge.** The late first touches cluster in one contiguous block: 3 to 6 hours, against a 7-minute median everywhere else. It looks like a batched delivery rather than a dialer problem, which is why the proposal instruments it before redesigning anything.
...
|---|---|
| **Lead ID** | What exactly happened to this one lead? |
...
Neon Postgres holds the two source tables exactly as delivered. All of the outcome logic lives in one version-controlled query, [`sql/ledger_v2.sql`](../sql/ledger_v2.sql), saved in Preset as the `lead_lifecycle_ledger` dataset, so **every number on screen traces back to a single query** rather than to ad-hoc chart-level logic. A second dataset, `attempt_detail`, holds the attempt-level view used by the attempt history table; it shares the `lead_id` column, which is what lets one Lead ID search narrow both.
